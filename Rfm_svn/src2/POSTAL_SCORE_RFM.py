...
class DBFilter():
            ...
            def filter(level):
                ...
                data =  list(db_client.RFM.MLSLITE_LIST_AGENT_unique_zip.distinct(level))
                print len(data)
                for i in data:
                    try:
                        print i
                        
                        agents = list(db_client.RFM.MLSLITE_LIST_AGENT_unique_zip.find({level:i},{"recenccy":1,"monetary":1,'AgentName':1,'Recent_transaction_date':1,"frequency":1}))

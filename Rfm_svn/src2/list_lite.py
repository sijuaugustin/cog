...
for row in list(db_client.MLSLite.MiamiMlsLite.aggregate(dist_agnt)):
            print  
            AGENTINFO=list(db_client.MLSLite.MiamiMlsLite.find_one({"ListAgentFullName":row['_id']},{"ListAgentKey":1,
                                                                    "ListAgentAOR" : 1,
            ...
            if n_trans>5:
                print 'pass'
                agent={"AgentName":row['_id'],"Transactions":Data_notrans,"number of Transactions":n_trans,"AgentInfo":AGENTINFO }
            #print Data[i]['NOT']
 
                print agent          
                db_client.RFM.miami.insert(agent)
                print 'pass'
            else:
                continue

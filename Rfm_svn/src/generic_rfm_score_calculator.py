...
class DBFilter():
            ...
            def filter(level):
                ...
                if level =='State':
                        data =  list(db_client.RFM.MLSLITE_LIST_AGENT_unique.distinct(level))
                else:
                        data =  list(db_client.RFM.MLSLITE_LIST_AGENT_unique.distinct("Transactions.SI_FIPS"))
                ...
                for i in data:
                    try:
                         print i
                         if level=='State':
                             agents = list(db_client.RFM.MLSLITE_LIST_AGENT_unique.find({level:i},{"R_Value":1,"M_Value":1,'AgentName':1,'last_transaction_date':1,"F_Value":1,'AgentInfo.ListAgentMlsId':1,'AgentInfo.ListAgentPreferredPhone':1,'AgentInfo.ListAgentEmail':1,'AgentInfo.ListAgentStateLicense':1,'AgentInfo.ListAgentCellPhone':1,}))
                         else:
                             agents = list(db_client.RFM.MLSLITE_LIST_AGENT_unique.find({'Transactions.SI_FIPS':i},{"R_Value":1,"M_Value":1,'AgentName':1,'last_transaction_date':1,"F_Value":1,'AgentInfo.ListAgentMlsId':1,'AgentInfo.ListAgentPreferredPhone':1,'AgentInfo.ListAgentEmail':1,'AgentInfo.ListAgentStateLicense':1,'AgentInfo.ListAgentCellPhone':1,}))
...
#                                 ful['last_transaction_date']=row['last_transaction_date']
#                                 #fuldata_.append(ful)
#                                  
#                                  
#                                  
...
# #                             #print x
                         ...
                         #print agent
                         if level=='State':
                             db_client["RFM_mlslite"][level].insert(agent)
                         else:
                             db_client["RFM_mlslite"]['COUNTY'].insert(agent)

...
agents= list(db_client.MLSLite.listing_unique.find({},{"ListAgentFullName":1,"_id":False}))
...
for i in range(len(agents)):
        try:
            print agents[i]
            AGENTINFO=db_client.MLSLite.listing_unique.find({"ListAgentFullName":agents[i]},{"ListAgentKey":1,
                                                                    "ListAgentAOR" : 1,
            ...
            if n_trans>5:
                agent={"AgentName":agents[i],"Transactions":Data_notrans,"number of Transactions":n_trans,"AgentInfo":AGENTINFO}
            #print Data[i]['NOT']
